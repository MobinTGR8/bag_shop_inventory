...
// Category colours and icons for visual badges
...
Color _bagTypeColor(String? bagType) {
  ...
  for (final entry in _categoryColors.entries) {
    if (bagType.toLowerCase().contains(entry.key.split(' ')[0].toLowerCase())) {
      return entry.value;
    }
  }
...
class _ProductListScreenState extends ConsumerState<ProductListScreen> {
        ...
        data: (products) {
          ...
          final filtered = products.where((p) {
            if (_selectedCategory != null) {
              final catMatch = p.bagType?.toLowerCase().contains(
                    _selectedCategory!.split(' ')[0].toLowerCase(),
                  );
              if (catMatch != true) return false;
            }
            if (query.isEmpty) return true;
            return '${p.name} ${p.sku} ${p.bagType ?? ''} ${p.barcode ?? ''}'
                .toLowerCase()
                .contains(query);
          }).toList();

          // Extract unique bag types for filter chips
          final allBagTypes = <String>{};
          for (final p in products) {
            if (p.bagType != null && p.bagType!.isNotEmpty) {
              bool matched = false;
              for (final cat in _categoryColors.keys) {
                if (p.bagType!.toLowerCase().contains(
                      cat.split(' ')[0].toLowerCase(),
                    )) {
                  allBagTypes.add(cat);
                  matched = true;
                  break;
          ...
          return AppBody(
            child: Column(
              children: [
                ...
                if (sortedBagTypes.isNotEmpty)
                  _buildFilterChips(sortedBagTypes, scheme, tt),
                      ...
                      if (_selectedCategory != null) ...[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: _bagTypeColor(_selectedCategory)
                                .withOpacity(0.1),
  ...
  Widget _buildFilterChips(
      List<String> bagTypes, ColorScheme scheme, TextTheme tt) {
    return SizedBox(
      height: 38,
              ...
              child: Row(
                ...
                children: [
                  Icon(
                    _bagTypeIcon(cat),
                    size: 14,
                    color: isSelected ? color : scheme.onSurface.withOpacity(0.5),
                  ),
...
class _ProductCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final profitMargin = product.unitCost > 0
        ? ((product.sellingPrice - product.unitCost) / product.unitCost * 100)
            .toStringAsFixed(0)
        : '0';
            ...
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      ...
                      colors: [
                        bagColor.withOpacity(0.7),
                        bagColor.withOpacity(0.3),
                      ],
                    ),
                    borderRadius: BorderRadius.circular(14),
                  ),
                        ...
                        children: [
                          ...
                          if (product.bagType != null) ...[
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: bagColor.withOpacity(0.08),
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      '$profitMargin% margin',
                      style: tt.bodySmall?.copyWith(
                        color: double.parse(profitMargin) > 20
                ...
                Icon(Icons.chevron_right,
                    size: 18, color: scheme.onSurface.withOpacity(0.2)),
              ],
            ),
          ),
        ),
